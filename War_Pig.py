                ...
                for row in csv_reader:
                    ...
                    if line != 0 and row[1] != '#ERROR!':
                        ...
                        for index, member in enumerate(def_ping_list): 
                            ...
                            try:
                                await user.send(content = f"We are under siege! You've been attacked. Please check <#{channel.id}> to coordinate with your peers. Thank you!")
                            except: 
                                await ctx.send(f'Could not DM {user.name} because they have disabled DMs with bots, please message them manually.')
...
@client.command()
async def create_chan(ctx, nation_link, war_type: Optional[int] = 0, reason: Optional[str] = "Counter", *members: Optional[discord.Member]):
    ''' 
...
async def help(ctx):
    ...
    if any(category.permissions_for(ctx.author).manage_channels for category in category_list_id):
        ...
        help_embed.add_field(name = '!bulk_update', value = f'Creates channels for all defensive wars and adds members in them.\n Using\
        ...
        help_embed.add_field(name = '!bulk_info', value = f'!war_info for every war chan', inline = False)
