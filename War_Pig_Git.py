'''
A bot used for war in the nation sim game Politics&War. Functionalities include:
...
Server_Alliance_ID = ''
Server_Alliance_Name = ''
Member_Role_ID ''

# Grab data from database
gaccount = gspread.service_account(filename='Service_Account.json')
...
async def on_ready():
    ...
    await client.change_presence(status=discord.Status.do_not_disturb, 
        activity=discord.Game("Destroying mad cash"))
...
async def bulk_create(ctx):
    ...
    category = discord.utils.get(
        ...
        name='[CANNAE BUT COUNTER]')
    back_up_category = discord.utils.get(
        ctx.guild.categories,
        name='[CANNAE BUT COUNTER 2]')

    # Sees if the user has permissions to manage channels in the category and
    # access bot
    if category.permissions_for(ctx.author).manage_channels:
        ...
        with open(f'csv/{attachment.filename}') as csv_file:
            ...
            for row in csv_reader:
                # Makes sure this is not the heading and it isn't an invalid
                # row before creating a channel
                if line != 0 and row[1] != '#ERROR!':
                    print(
                        f'{row[1]} is the target. Attacker 1 is {row[4]},'
                        f'attacker 2 is {row[6]},'
                        'and attacker 3 is {row[8]}')
                    target_name = row[1]
                    target_id = row[2]
                    attackers = [row[4], row[6], row[8]]
                    defenders = [row[11], row[13], row[15], row[17], 
                    row[19]]
                    channel_name = target_name.replace(
                        ' ', '-') + '-' + target_id
                    ...
                    try:
                        channel = await ctx.guild.create_text_channel(channel_name,
                            category=category,
                            topic=f'War on {row[0]}')

                    except discord.HTTPException:
                        channel = await ctx.guild.create_text_channel(channel_name,
                            category=back_up_category,
                            topic=f'War on {row[0]}')

                    war_embed = discord.Embed(
                        title=f"⚔️ __Target: {' '.join(channel_name.split('-')[:-1])}__",
                        description=f"Please declare war on {row[0]}",
                        color=0xcb2400,
                        url=f'https://politicsandwar.com/nation/war/declare/'
                        f'id={row[0].split("=")[1]}')

                    mil_count = get_pnw_mil(
                        f'https://politicsandwar.com/nation/'
                        f'id={row[0].split("=")[1]}')

                    war_embed.add_field(
                        name='__Military Information:__',
                        value=f'{channel_name.split("-")[0]} has '
                        f'{mil_count["Soldiers"]} soldiers, '
                        f'{mil_count["Tanks"]} tanks, '
                        f'{mil_count["Planes"]} planes, and '
                        f'{mil_count["Ships"]} ships',
                        inline=False)

                    ping_list = await coord_perms(attackers, 
                                                  channel, 
                                                  channel_name, 
                                                  ctx)
                    # Gets DM list for members to attack and puts out their
                    # nation link
                    for index, member in enumerate(ping_list):
                        link = 'https://politicsandwar.com/nation/'
                        f'id={nation_dict.get(member, "N/A")}'
                        user = client.get_user(member)
                        war_embed.add_field(
                            name=f"__Attacker {index + 1}:__",
                            value=f"[{user.display_name}]({link})",
                            inline=True)
                        try:
                            await user.send(content="It's time to send in the"
                            f"elephants! Please check<#{channel.id}> for your "
                            "war assignment. Thank You!")
                        except BaseException:
                            await ctx.send(f'Could not DM {user.name} because '
                            'they have disabled DMs with bots, please message'
                            'them manually.')
                    def_ping_list = await coord_perms(defenders, 
                                                      channel, 
                                                      channel_name, 
                                                      ctx)

                    # Gets DM list for defending members and puts out their
                    # nation link
                    for index, member in enumerate(def_ping_list):
                        link = f'https://politicsandwar.com/nation/\
                        id={nation_dict.get(member, "N/A")}'
                        user = client.get_user(member)
                        war_embed.add_field(
                            name=f"__Defender {index + 1}:__",
                            value=f"[{user.display_name}]({link})",
                            inline=True)
                        try:
                            await user.send(content=f"{Server_Alliance_Name}\
                            is under siege! You've been attacked. Please\
                            check <#{channel.id}> to coordinate with your\
                            peers. Thank you!")
                        except BaseException:
                            await ctx.send(f'Could not DM {user.name} because \
                            they have disabled DMs with bots, please \
                            message them manually.')

                    war_embed.add_field(
                        name="__Reminder__",
                        value="1.) Make sure you have enough resources including\
                              food and uranium, ping gov if you need more\n\
                              2.) Look over their military before going in\
                              and plan out the best moven\n\
                              3.) Talk and coordinate with fellow members, \
                              declare at the same time and help each other\n\
                              Good luck!",
                        inline=False)
                    await channel.send(f'{ping(ping_list)}{ping(def_ping_list)}', 
                        embed=war_embed)

                    # Gets the permissions for the members set up and pings
                    # them

                line += 1

            await ctx.send('Channels are finished being create, good luck\
             in the wars to come!')
...
async def create_chan(ctx, nation_link, reason=None, *members: discord.Member):
    ...
    if category.permissions_for(ctx.author).manage_channels:
        ...
        if(re.search(r'politicsandwar.com/nation/id=\d{1,7}', nation_link)):
            channel_name = get_pnw_name(nation_link).replace(
                ' ', '-') + '-' + nation_link.split('=')[1]
            ...
            war_embed = discord.Embed(
                title=f"⚔️ __Target: {' '.join(channel_name.split('-')[:-1])}__",
                description=f"Please declare war on {nation_link}{reason}",
                color=0xcb2400,
                url=f'https://politicsandwar.com/nation/war/declare/id={nation_link.split("=")[1]}')

            mil_count = get_pnw_mil(
                f'https://politicsandwar.com/nation/id={nation_link.split("=")[1]}')

            war_embed.add_field(
                name='__Military Information:__',
                value=f'{channel_name.split("-")[0]} has\
                {mil_count["Soldiers"]} soldiers,\
                {mil_count["Tanks"]} tanks,\
                {mil_count["Planes"]} planes, and\
                {mil_count["Ships"]} ships',
                inline=False)

            for index, member in enumerate(members):
                link = f'https://politicsandwar.com/nation/\
                id={nation_dict.get(member.id, "N/A")}'
                war_embed.add_field(
                    name=f"__Attacker {index + 1}:__",
                    value=f"[{member.display_name}]({link})",
                    inline=True)
        ...
        # Lets user know that nation_link is in wring format
            ...
            await ctx.send('Nation link format is wrong, must be\
             politicsandwar.com/nation/id=xxxxx')
    ...
    if category.permissions_for(ctx.author).manage_channels:
        ...
        for channel in category.channels:
            ...
            try:
                ...
                # Goes through a nation's wars and see if they still have wars
                # with {Server_Alliance_Name}
                for war in get_war_info(nation_link):
                    if (war['Aggressor Alliance'] == f'{Server_Alliance_Name}')
                    or (war[ 'Defender Alliance'] == f'{Server_Alliance_Name}'):
                        active_war = True
                        break

                if not active_war:
                    await ctx.send(f'The war channel {channel.name} \
                        has been deleted.')
                    await channel.delete(reason=f"No existing\
                        {Server_Alliance_Name} wars with this nation")
            except AttributeError:
                await ctx.send(f"The war channel {channel.name} does not\
                    have a topic, thus we are unable to determine if war has\
                    been expired")

            except BaseException:
                await ctx.send(f'Unexpected error has occured with \
                    {channel.name}')
    # Tells them if they don't have the permission to create the channel
    else:
        await ctx.send('You do not have permissions to create war channels')
...
async def war_info(ctx):
    ...
    try:
        ...
        sheet = discord.File('spreadsheet/War.xlsx', 
            filename="war_sheet.xlsx")
        await message.delete()
        await ctx.send(file=sheet)

    # Except block
    except AttributeError:
        await ctx.send(f"The war channel {ctx.channel.name}\
         does not have the correct topic format. We can not grab information\
          for the war.")
...
async def graph(ctx, type, *alliances):
    ...
    if re.search('hist', type):
        ...
        for ally in alliances:
            await message.edit(content=f'Gathering information from \
                {ally.replace("+", " ").title()}')
            print(ally)
            res = requests.get(
                f'https://politicsandwar.com/index.php?id=15&keyword={ally}\
                &cat=alliance&ob=score&od=DESC&maximum=15&minimum=0\
                &search=Go&memberview=true')
            ...
            if num_nations == 0:
                await ctx.send(f'Could not find any nations in the alliance\
                    {ally.replace("+", " ").title()}, \
            ...
            for nations in range(0, math.ceil(num_nations / 50)):
                res = requests.get(
                    f'https://politicsandwar.com/index.php?id=15&keyword={ally}\
                    &cat=alliance&ob=score&od=DESC&maximum={50*(nations+1)}\
                    &minimum={50*nations}&search=Go&memberview=true')
                soup_data = BeautifulSoup(res.text, 'html.parser')
                data = soup_data.find_all(
                    "td",
                    attrs={
                        "class": "right"},
                    text=re.compile(r'^[1-9]\d*$'))
        ...
        n_bins = max(list(map(lambda x: np.amax(x), alliances_data))).astype(
            np.int64) + 1
    ...
    # Graphs for scatter plot with military informationS
        ...
        milplot_array = np.array([['Alliance',
                                   'Leader Name',
                                   'Age',
                                   'Cities',
                                   'Soldiers Killed',
        ...
        for ally in html:
            ...
            if num_nations == 0:
                await ctx.send(f'Could not find any nations in the alliance\
                 {ally.replace("+", " ").title()}, make sure it is spelled\
                  correctly')
                label.remove(ally)
                continue
            ...
            for nations in range(0, math.ceil(num_nations / 50)):
                res = requests.get(
                    f'https://politicsandwar.com/index.php?id=15&keyword={ally}'
                    f'&cat=alliance&ob=score&od=DESC&maximum={50*(nations+1)}'
                    f'&minimum={50*nations}&search=Go&memberview=true')
                soup_data = BeautifulSoup(res.text, 'html.parser')
                data = soup_data.find_all(
                    "a", href=re.compile("politicsandwar.com/nation/id="))
                links = []
                ...
                for nation_link in links:
                    ...
                    # Creates dataframe
                    member_info = [
                        mil['alliance'], mil['leadername'],
                        int(mil['daysold']),
                        mil['cities'],
                        int( mil['soldierskilled']),
                        int(mil['soldiercasualties']),
                        int(mil['tankskilled']),
                        int(il['tankcasualties']),
                        int(mil['aircraftkilled']), 
                        int(mil['aircraftcasualties']),
                        int(mil['shipskilled']),
                        int(mil['shipcasualties']),
                        float(mil['infdesttot']),
                        int(mil['infraLost']),
                        float(mil['moneyLooted'])]
        ...
        mil = pd.DataFrame(milplot_array[1:], range(
            len(milplot_array) - 1), milplot_array[0])
        ...
        for image in stuff_to_graph:
            ...
            pic = discord.File(
                f"Scatter Plot Of {image}.png",
                filename=f"Scatter Plot Of {image}.png")
            await ctx.send(file=pic)
   # If type is something weird
    else:
        await ctx.send(f'We are unable to identify graph type: {type}.' 
            'The valid graph types are hist/hist-static and scatter.')
...
async def add(ctx, type, reason=None, *nations):
    ...
    # Clears the war reason if it doesn't exist and in place is a nation
    # link/ID
    ...
    if type == 'atk' or type == 'attacker' or type == 'attack':
        ping = await add_to_chan(ctx, nations)
        # If you couldn't find any of the members there is no point to ping, so
        # this check exists
        if len(ping) != 0:
            ping_list = ' '.join([f'<@{member}>' for member in ping])
            await ctx.send(f'{ping_list} please read above and declare war on\
             {ctx.channel.topic.split()[2]}. {reason}')

    # If members to be added are defenders
    elif type == 'def' or type == 'defender' or type == 'defend':
        ping = await add_to_chan(ctx, nations)
        # If you couldn't find any of the members there is no point to ping, so
        # this check exists
        if len(ping) != 0:
            ...
            await ctx.send(f'{ping_list} is defending. Please coordinate '
                'with the other {Server_Alliance_Name} members here for the war '
                f'against {target.title()}.')

    # If it is an invalid type of war
    else:
        await ctx.send('Invalid command format. Please do !add <atk/def> '
            '<nation link/id> <nation link/id> etc.')


@client.command()
@commands.has_role(Member_Role)
async def find_targets(ctx, 
    member, 
    target_alliance, 
    ground_max_percent=120, 
    ground_min_percent=40, 
    air_max_percent=120, 
    air_min_percent=40):
    '''
    ...
    if(re.search(r'politicsandwar.com/nation/id=\d{1,7}', member)):
        ...
        try:
            ...
            await ctx.send('API key ran out or the member is not a valid '
                'nation ID or link')
            return
    ...
    res = requests.get(
        f'https://politicsandwar.com/index.php?id=15&keyword={target_alliance}\
        &cat=alliance&ob=score&od=DESC&maximum=15&minimum=0\
        &search=Go&memberview=true&beige=true')
    soup_data = BeautifulSoup(res.text, 'html.parser')
    data = soup_data.find(text=re.compile('Showing'))
    num_nations = float(data.split()[3])
    ...
    if num_nations == 0:
        await ctx.send(f'Could not find any nations in the alliance\
         {target_alliance.replace("+", " ").title()}, make sure it is\
          spelled correctly')
        return
    ...
    for nations in range(0, math.ceil(num_nations / 50)):
        res = requests.get(
            f'https://politicsandwar.com/index.php?id=15&\
            keyword={target_alliance}\
            &cat=alliance&ob=score&od=DESC&\
            maximum={50*(nations+1)}\
            &minimum={50*nations}\
            &search=Go&memberview=true&beige=true')
        soup_data = BeautifulSoup(res.text, 'html.parser')
        data = soup_data.find_all(
        ...
        for row in rows[1:]:
            ...
            try:
                ...
                min_range = member_info['score'] * 0.75
                max_range = member_info['score'] * 1.75
                if(min_range <= score <= max_range):
                    nation_id = cells[1].find('a')['href'].split('=')[1]
                    alliance_nations_in_range[nation_id] = [
                        cells[1].find('a').text, int(cells[5].text), score]
            except BaseException:
                pass
    target_embed = discord.Embed(
        title=f"🎯 __Potential Targets for {member_info['leadername']}__",
        description=f'{member_info["leadername"]} has \
            {member_info["soldiers"]} soldiers, \
            {member_info["tanks"]} tanks,\
            {member_info["aircraft"]} planes, \
            and {member_info["ships"]} ships.')
    ...
    for nation_id in alliance_nations_in_range.keys():
        ...
        for cell in cells:
            if cell.find('br') is not None and isinstance(
                    cell.contents[0], str):
                mil_values.append(int(cell.contents[0].replace(',', '')))

        # Checks if it matches the parameters highlighted in command
        if (mil_values[0] + mil_values[1] * 23 <= (member_info['soldiers']\
        + member_info['tanks'] * 23) * float(ground_max_percent) / 100) and\
        (mil_values[0] + mil_values[1] * 23 >= (member_info['soldiers']\
        + member_info['tanks'] * 23) * float(ground_min_percent) / 100) and\
        mil_values[2] <= member_info['aircraft'] * float(air_max_percent)\
        / 100 and mil_values[2] >= member_info['aircraft'] *\
        float(air_min_percent) / 100:
            ...
            if not len(potential_targets) > 9:
                ...
                target_embed.add_field(
                    ...
                    value=f'[{potential_targets[nation_id][0]}]({link})\
                     \n{potential_targets[nation_id][1]} cities •\
                      {potential_targets[nation_id][3]} soldiers •\
                       {potential_targets[nation_id][4]} tanks •\
                        {potential_targets[nation_id][5]} planes •\
                         {potential_targets[nation_id][6]} ships',
                    inline=True)

    await loading_msg.delete()
    target_embed.set_footer(
        text=f'Page 1/{int(math.ceil(len(potential_targets)/9))}')
    ...
    while True:
        ...
        if emoji == '\u25c0':
            if i > 0:
                i -= 1
                target_embed.clear_fields()
                for nation_id in list(
                    potential_targets.keys())[
                    i *
                    9:min(
                        i *
                        9 +
                    ...
                    target_embed.add_field(
                        ...
                        inline=True)
                target_embed.set_footer(
                    text=f'Page {i+1}/{int(math.ceil(len(potential_targets)/9))}')
                await find_targets_msg.edit(embed=target_embed)
        # If right arrow
        if emoji == '\u25b6':
        ...
        try:
            res = await client.wait_for('reaction_add',
                        ...
                        check=lambda 
                        reaction, 
                        user: reaction.message.id == find_targets_msg.id)
...
async def find_counters(ctx, target, ground_max_percent=math.inf, ground_min_percent=80, air_max_percent=math.inf, air_min_percent=80):
    ...
    res = requests.get(
        f'https://politicsandwar.com/index.php?id=15\
        &keyword={Server_Alliance_Name}\
        &cat=alliance&ob=score&od=DESC\
        &maximum=15&minimum=0&search=Go&memberview=true')
    soup_data = BeautifulSoup(res.text, 'html.parser')
    data = soup_data.find(text=re.compile('Showing'))
    num_nations = float(data.split()[3])
    ...
    for nations in range(0, math.ceil(num_nations / 50)):
        res = requests.get(
            f'https://politicsandwar.com/index.php?id=15\
            &keyword={Server_Alliance_Name}&cat=alliance\
            &ob=score&od=DESC&maximum={50*(nations+1)}\
            &minimum={50*nations}&search=Go&memberview=true')
        soup_data = BeautifulSoup(res.text, 'html.parser')
        data = soup_data.find_all(
            "td",
        ...
        for row in rows[1:]:
            ...
            if(target_info['score'] * (1 / 1.75) <= score <= target_info['score'] * (1 / 0.75)):
                alliance_nations_in_range[cells[1].find('a')['href'].split('=')[1]] = [
                    cells[1].find('a').text, int(cells[5].text), score]

    counter_embed = discord.Embed(
        title=f"🎯 __Potential Counter for {target_info['leadername']}\
         ({target_info['slots']}):__",
        description=f'{target_info["leadername"]} has\
         {target_info["soldiers"]} soldiers, {target_info["tanks"]} tanks,\
          {target_info["aircraft"]} planes, and {target_info["ships"]} ships.')
    ...
    for nation_id in alliance_nations_in_range.keys():
        ...
        mil_values = []
        cells = soup_data.find("table",
                               {'class': 'nationtable'}).find_all('td')
        for cell in cells:
            if cell.find('br') is not None and isinstance(
                    cell.contents[0], str):
                mil_values.append(int(cell.contents[0].replace(',', '')))

        if (mil_values[0] + mil_values[1] * 23 <= max((target_info['soldiers']\
        + target_info['tanks'] * 23), 0.01) * float(ground_max_percent) / 100) and\
        (mil_values[0] + mil_values[1] * 23 >= (target_info['soldiers'] +\
        target_info['tanks'] * 23) * float(ground_min_percent) / 100) and\
        mil_values[2] <= max(target_info['aircraft'], 0.01) *\
        float(air_max_percent) / 100 and mil_values[2]\
        >= target_info['aircraft'] * float(air_min_percent) / 100:
            ...
            if not len(potential_counters) > 9:
                ...
                counter_embed.add_field(
                    ...
                    value=f'[{potential_counters[nation_id][0]}]({link})\
                     \n{potential_counters[nation_id][1]} cities •\
                      {potential_counters[nation_id][3]} soldiers •\
                       {potential_counters[nation_id][4]} tanks •\
                        {potential_counters[nation_id][5]} planes •\
                         {potential_counters[nation_id][6]} ships',
                    inline=True)

    counter_embed.set_footer(
        text=f'Page 1/{int(math.ceil(len(potential_counters)/9))}')
    ...
    while True:
        ...
        if emoji == '\u25c0':
            if i > 0:
                ...
                counter_embed.clear_fields()
                for nation_id in list(
                    potential_counters.keys())[
                    i *
                    9:min(
                        i *
                        9 +
                        9,
                        len(potential_counters))]:
                    link = f'https://politicsandwar.com/nation/id={nation_id}'
                    counter_embed.add_field(
                        name='\u200b',
                        value=f'[{potential_counters[nation_id][0]}]({link})\
                        ...
                        inline=True)
                counter_embed.set_footer(
                    text=f'Page {i+1}/{int(math.ceil(len(potential_counters)/9))}')
                await find_counters_msg.edit(embed=counter_embed)
        if emoji == '\u25b6':
        ...
        if emoji == '\u23ed':
            ...
            await find_counters_msg.edit(embed=counter_embed)
        try:
            res = await client.wait_for('reaction_add', timeout=90, 
                check=lambda reaction, user: reaction.message.id == find_counters_msg.id)
            if str(res[1]) != 'War Pig#1807':
                emoji = str(res[0].emoji)
...
async def coord_perms(members, channel, channel_name, ctx):
    ...
    for member in members:
        ...
        if member != '#ERROR!' and member != '' and member != '#VALUE!':
            ...
            if discord_name != '':
                await channel.set_permissions(ctx.guild.get_member(discord_name),
                 read_messages=True, 
                 send_messages=True)
                people_to_ping.append(discord_name)
            # Tells gov to manually add users if they're invalid
            else:
                await ctx.send(f"Couldn't find the Discord for {member},\
                 please add them manually to {channel_name}")
        # Stops the loop if invalid users come up because that means the end of
        # the list
        else:
            return people_to_ping
            break
    # Returns it if there were no invalid users and the loop has reached its
    # end
    else:
        return people_to_ping
...
async def add_to_chan(ctx, nations):
    ...
    for nation in nations:
        # If it is a link
        if(re.search(r'politicsandwar.com/nation/id=\d{1,7}', nation))\
         and int(nation.split('=')[1]) in rev_nation_dict:
            member = ctx.guild.get_member(
                rev_nation_dict[int(nation.split('=')[1])])
        ...
        # If it is an ID
            ...
            await ctx.send(f"Couldn't find member {nation}, either the member\
             sheet is not updated or it is not a nation link/nation ID.")
    return members
...
async def help(ctx):
    ...
    help_embed.add_field(
        name='!find_targets', value=f'Finds a list of targets in an alliance\
         within range and military capabilities.\n Parameters\
        are <member nation id or link> <target alliance> <ground max %>\
         <ground min %> <air max %> <air min %> (default is 120% max and\
          40% min, not neccessary to fill in)\
        \n__Example__: !find_targets 48730 The+Knights+Radiant 150 90 170 80\
         finds all TKR nations in range with 150-90% of my ground and 170-80%\
          of my planes', inline=False)

    help_embed.add_field(
        name='!find_counters',
        value=f'Finds a list of targets in {Server_Alliance_Name} within range\
         and military capabilities to counter.\n Parameters\
        are <target nation id or link> <ground max %> <ground min %>\
         <air max %> <air min %> (default is infinity% max and 80% min,\
          not neccessary to fill in)\
        \n__Example__: !find_counters 48730 150 90 170 80 finds all\
         {Server_Alliance_Name} nations in range with 150-90% of my ground\
          and 170-80% of my planes',
        inline=False)
    ...
    if category.permissions_for(ctx.author).manage_channels:
        ...
        help_embed.add_field(
            name='**__Milcom Specific Commands:__**',
            value='\u200b',
            inline=False)
        help_embed.add_field(
            name='!create_chan',
            value=f'Creates a channel for war.\n Parameters\
            are <target nation id or link> <@member1> <@member2> etc\
            \n__Example__: !create_chan 48730 @Daveth#0674 @Kraмpus#0001\
             creates a channel telling them to declare on Piglantia',
            inline=False)

        help_embed.add_field(
            name='!bulk_create',
            value=f'Uses a CSV sheet to create a list of coordination channels.\
            \n Parameter is a CSV sheet using this format\
             https://docs.google.com/spreadsheets/d/\
             1Fo-wEUWkslONQE5tyIkLQ6Ubc3paPUMurx1SgQz8OFo/edit?usp=sharing',
            inline=False)

        help_embed.add_field(
            name='!add',
            value=f'Adds attacker or defender to channel.\n Parameters are\
            <atk or def> <member id/link> \n Example: !add atk 48730\
            adds Piglantia to channel as attacker',
            inline=False)

        help_embed.add_field(
            name='!war_info',
            value=f'Use in war channel to get excel sheet of MAPs and\
             resistance of all wars target is in.',
            inline=False)

        help_embed.add_field(
            name='!clear_expired',
            value=f'Deletes all war channels which no longer have active\
             {Server_Alliance_Name} wars',
            inline=False)

        help_embed.add_field(
            name='!graph', value=f'Creates graphs of the alliances\n Parameters\
            are <scat or histk> <alliance 1> <alliance 2> (use + to\
             seperate spaces)\
            \n__Example__: !graph scat {Server_Alliance_Name} House+Stark\
             creates a scatter graph of military kills and highlights\
              outliers to be used as prio targets\n\
            !graph hist {Server_Alliance_Name} returns histogram of\
             city count of {Server_Alliance_Name}', inline=False)
...
def ann(df, value):
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(
        df['Age'], df[value])
    std_resid = math.sqrt(
        (sum((df[value] - slope * df['Age'])**2)) / (len(df.index) - 2))
    ...
    for row in outliers.iterrows():
        r = row[1]
        plt.gca().annotate(
            f'{r["Leader Name"]}, {r["Cities"]}', xy=(
                r['Age'], r[value]), xytext=(
...
async def war_info_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.send(f'The command is on cooldown for this channel,\
         please try again in {error.retry_after:.3g} seconds')


@graph.error
async def graph_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.send(f'The command is on cooldown for you,\
         please try again in {error.retry_after:.3g} seconds')
